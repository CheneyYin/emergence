...
| 会话持久化 + 别名 | 用户自定义命令 |
| 斜杠命令系统 | 记忆系统 |
| AGENTS.md 项目配置 | 插件/扩展系统 |
| Skill 系统 | |
| Hook 系统 (Shell + Builtin) | |
...
Hook 系统由两层组成：**Shell Hook**（外部进程）和 **Built-in Listener**（内建监听器）。两者在同一事件模型中注册，触发时按序执行。
...
enum HookEvent {
    SessionStart,
    SessionEnd,
    PreToolExecute   { tool: String, params: Value },
    PostToolExecute  { tool: String, result: ToolOutput },
    UserInput        { text: String },
    PreLLMCall       { messages: Vec<ChatMessage> },
    PostLLMCall      { response: String, usage: Usage },
  ...
  "hooks": [
    {
      ...
      "type": "builtin",
      "listener": "log",
      "config": { "path": "~/.emergence/hooks.log", "format": "json" }
    },
    {
      "event": "SessionEnd",
      "type": "shell",
      "command": "notify-send 'emergence session ended'",
      "timeout_ms": 5000
    },
    {
      "event": "PreToolExecute",
      "type": "builtin",
      "listener": "validate-tool",
      "config": {
        "rules": [
          { "tool": "bash", "deny_patterns": ["rm -rf /"] }
        ]
      }
    }
  ]
...
trait HookExecutor: Send + Sync {
    ...
    async fn execute(&self, event: &HookEvent) -> Result<HookOutcome>;
}

struct HookRegistry {
    listeners: HashMap<HookEvent, Vec<Box<dyn HookExecutor>>>,
}
```
...
classDiagram
    ...
    class RateLimitListener {
        +count: HashMap
        +execute() check threshold
    }

    class HookRegistry {
        +load(config_path) HookRegistry
        +register(event, executor)
        +dispatch(event) Vec~HookOutcome~
    }

    HookRegistry --> HookExecutor
    HookExecutor <|.. ShellExecutor
    HookExecutor <|.. LogListener
    HookExecutor <|.. ValidateToolListener
    HookExecutor <|.. NotifyListener
    HookExecutor <|.. RateLimitListener
```
...
| 监听器 | 适合事件 | 功能 |
|--------|---------|------|
| `log` | 任意 | 将事件序列化为 JSON，写入指定文件 |
| `validate-tool` | PreToolExecute | 根据规则校验工具参数，匹配 deny_patterns 时返回 Abort |
| `notify` | 任意 | 调用系统通知 |
| `rate-limit` | PreToolExecute | 限制单 session 内工具调用次数 |

**Shell 执行器：**

- 事件上下文序列化为 JSON，通过 stdin 传入子进程
- 模板变量 `{{tool}}`、`{{params}}` 在 command 中替换为实际值
- stdout 写入 hook 日志，stderr 以 warn 级别记录
- 超时后 SIGKILL，返回 `Continue`

**执行流程：**

```mermaid
flowchart TD
    Event["Agent 触发事件"] --> Dispatch["HookRegistry::dispatch(event)"]
    Dispatch --> Loop["迭代已注册的 hook"]
    Loop --> CheckType{"executor 类型?"}
    CheckType -->|"Shell"| Spawn["spawn 子进程
JSON 通过 stdin 传入"]
    CheckType -->|"Builtin"| Invoke["同步调用
in-process listener"]
    Spawn --> CheckOutcome{"结果?"}
    Invoke --> CheckOutcome
    CheckOutcome -->|"Continue"| Next{"还有 hook?"}
    CheckOutcome -->|"Abort"| Abort["中断事件链
返回拒绝/阻止"]
    Next -->|"是"| Loop
    Next -->|"否"| Done["事件处理完成"]
```
...
sequenceDiagram
    ...
    participant BL as BuiltinListener
    participant TR as ToolRegistry

    AL->>AL: 准备执行工具
    AL->>HR: dispatch(PreToolExecute)
    HR->>BL: execute(event)
    BL-->>HR: Continue
    HR->>SE: execute(event)
    SE-->>HR: Continue
    HR-->>AL: passed
    AL->>TR: execute(tool, params)
    TR-->>AL: ToolOutput
    AL->>HR: dispatch(PostToolExecute)
    HR->>BL: execute(event)
    BL-->>HR: Continue
    HR-->>AL: done
```
...
```
~/.emergence/hooks.json       ← 用户级
./.emergence/hooks.json       ← 项目级（合并，不覆盖）
```

两级 hooks 配置合并生效，用户级和项目级 hook 均注册。
...
│   │   └── loader.rs        # 文件扫描，两级目录合并
│   ├── hooks/
│   │   ├── mod.rs           # HookEvent, HookRegistry, HookExecutor trait
│   │   ├── shell.rs         # ShellExecutor
│   │   └── builtin.rs       # LogListener, ValidateToolListener, etc.
│   └── utils/
│       ├── fuzzy.rs         # 编辑距离匹配
│       └── env.rs           # 环境变量展开
└── tests/
