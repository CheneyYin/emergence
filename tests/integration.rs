...
use emergence::session::store::{SessionStore, JsonFileStore};
...
fn test_load_settings_from_file() {
    ...
    let config = ConfigManager::load(dir.path().to_path_buf(), dir.path().to_path_buf(), None).unwrap();
...
fn test_cli_model_overrides_settings() {
    ...
    let config = ConfigManager::load(dir.path().to_path_buf(), dir.path().to_path_buf(), Some("cli-override-model".into())).unwrap();
...
fn test_session_manager_context_building() {
    ...
    let tools = vec![emergence::llm::ToolDefinition {
        name: "read".into(), description: "read files".into(),
    ...
    let user_msgs: Vec<_> = ctx.iter().filter(|m| m.role == emergence::llm::Role::User).collect();
...
fn test_bash_risk_classification() {
    ...
    assert_eq!(BashTool.risk_level(&serde_json::json!({"command": "ls -la"})), RiskLevel::ReadOnly);
    assert_eq!(BashTool.risk_level(&serde_json::json!({"command": "cargo build"})), RiskLevel::Write);
    assert_eq!(BashTool.risk_level(&serde_json::json!({"command": "sudo rm -rf /"})), RiskLevel::System);
}
